...
public class UpdatePublicationIdentifierCommandHandler : IRequestHandler<UpdatePublicationIdentifierCommand, UpdatePublicationIdentifierCommandResponse>
{
	...
	{
		var updatedPublication = await _repository.GetPublicationIdentifierByIdAsync(request.Id);
		...
		int updatedRows = await _repository.UpdatePublicationIdentifierAsync(updateModel);
